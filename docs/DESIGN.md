...
发现仓库页面：可按用户提示词搜索 GitHub 仓库，也可从公开 GHSA 筛可审计仓；关键词粗分后再用模型复核审计对象；已创建与可创建分开列出，可单条或一键移除候选。
...
| GitHub 发现仓库 | 可填用户提示词，由模型优先按意图构造 GitHub 搜索并筛选候选；留空则从公开 GHSA 筛星标与活跃度达标的仓库。关键词粗分后再用模型单轮复核 `target_kind`。已创建与可创建分开列出，可单条或一键移除候选使其不再出现 |
