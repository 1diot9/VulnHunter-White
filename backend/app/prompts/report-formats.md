...
- `## 漏洞描述` 只写两句：产品功能定位一句话简介，加上漏洞成因概要。不要在本节展开利用步骤或危害（放到「漏洞危害」与「漏洞技术细节」）。SQL 注入须在危害中说明是否能获取 OS-Shell。SSRF 须在危害中写明观察面：有回显 / 外带内网信息 / 仅响应差别（内网端口探测）。有回显与外带内网信息危害同级。
...
- 仅当挖掘路径为历史漏洞绕过时，还须在上一节全部章节基础上对齐 `templates/vuln-report-bypass.md`：在 `## 漏洞技术细节` 下**第一节**为 `### 补丁绕过简析`（原漏洞与补丁、当前绕过或未修复原因），其后依次为 `### Source → Sink`、`### 漏洞代码`、`### 完整 PoC 描述`、`### 触发条件`。不要用自由格式短文代替完整报告。绕过报告的漏洞成因概要须说明与历史漏洞文档的关系。
