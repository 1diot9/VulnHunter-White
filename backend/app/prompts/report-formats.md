...
- 必须为中文，结构对齐 `templates/vuln-report.md`，至少包含：`## 漏洞描述`、`## 漏洞危害`、`## 漏洞厂商全称`、`## 已知受影响产品及版本`、`## 互联网资产证明`、`## 漏洞技术细节`、`## 同根因受影响点`、`## 复现证明`、`## 修复方案`、`## 备注`。不要写 `## 摘要`（与漏洞描述定位重复）。
...
- `## 漏洞描述` 只写两三句话：产品功能定位一句话简介，加上一两句成因概要（缺了什么校验 / 输入如何到达 sink）。不要在本节展开原理长文、利用步骤或危害（细节放到「漏洞技术细节」，危害放到「漏洞危害」）。SQL 注入须在危害中说明是否能获取 OS-Shell。SSRF 须在危害中写明观察面：有回显 / 外带内网信息 / 仅响应差别（内网端口探测）。有回显与外带内网信息危害同级。
...
- 仅当挖掘路径为历史漏洞绕过时，还须在上一节全部章节基础上对齐 `templates/vuln-report-bypass.md`：在 `## 漏洞技术细节` 下**第一节**为 `### 补丁绕过简析`（原漏洞与补丁、当前绕过或未修复原因），其后依次为 `### Source → Sink`、`### 漏洞代码`、`### 完整 PoC 描述`、`### 触发条件`。不要用自由格式短文代替完整报告。绕过报告的成因概要可一句带过与历史漏洞文档的关系，补丁对照写在 `### 补丁绕过简析`，不要在漏洞描述里展开。
...
- 必须为英文 GitHub Advisory 填表稿，结构对齐 `templates/vuln-advisory.md`，至少包含：`## Title`、`## Description`（`### Summary` / `### Details` / `### Vulnerable code` / `### PoC` / `### Impact`）、`## Affected products`、`## Severity / CWE`（含 **CVSS 3.1** 与 **CVSS 4.0** 向量字符串；基础分与严重度标签由 ConfirmVuln 按向量自动计算，不要手填分数；不确定时留空向量由 Reviewer 填）。
...
- CVSS 写 `containers.cna.metrics[0].cvssV3_1.vectorString`（`CVSS:3.1/...`）和 `containers.cna.metrics[0].cvssV4_0.vectorString`（`CVSS:4.0/...`）；`baseScore` / `baseSeverity` 由系统计算，手填分数或 3.0 路径会被拒绝。
