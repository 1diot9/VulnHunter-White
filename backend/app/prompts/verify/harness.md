...
5. 判定：
   - 沙箱跑通且默认部署下攻击者可单独打出有害冲击 → `ConfirmVuln(evidence_level=harness)`。把 harness 写入 `vulns/{id}/harness.py`（或对应语言文件；Confirm 可传 `harness_code`）。**不要**把 mock 脚本写进 `poc.py`（`poc.py` 仍是 `-u/--url` + `--proxy` 的 HTTP 合同）。
