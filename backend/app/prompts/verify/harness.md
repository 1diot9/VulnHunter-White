...
3. 用 `RunCode` 在一次性沙箱里执行（Python / PHP / JS / Ruby / Go / Java / Bash / C）。**C 用 `language=c`（gcc + glibc，没有 OpenSSL 等第三方库）**。**没有 rustc / g++**；`language=rust` / `c++` 或探测 rustc 会得到 `unsupported_language`，此时 `ConfirmVuln(evidence_level=static_only)`，不要反复 RunCode，也不要因此误报、不要用另一种语言复述 Rust。不要在本机 Bash/PowerShell 里跑 harness。**脚本自己打印的 stdout/stderr（标签、步骤、判定）须中英双语：默认英语，须提供 `--zh` 切中文**（Python `argparse`；其它语言扫 argv / `process.argv` / `os.Args`）。**JavaScript 对照表必须写成 `[en, zh]` 数组，禁止 `(en, zh)`**（逗号运算符只会留下中文，`msg()` 再按字符解构会打成「步 / 骤」这种单字）。注释 / docstring / `--help` 仍用英语；源码片段、payload、目标回显原文不要翻译。
...
6. **Java harness 默认 JDK 8**：按 Java 8 语言级别与 API 写（不要用 `var`、record、text block、`List.of`/`Map.of`、`switch` 表达式等 9+ 语法，也不要调用 JDK 9+ 才有的 API）。沙箱 `javac` 默认 `--release 8 -encoding UTF-8`，源码可直接写 UTF-8。**不要在注释里写 `\uXXXX`**（javac 会在词法分析前展开 `\u`，注释也不例外）。仅当目标源码本身明确需要更高版本（`pom.xml` / `compiler.source` 为 11/17，或抽出片段用了对应语法）时，才提高到该版本，并在源码顶部写 `// java-release: 11` 或 `// java-release: 17`。
