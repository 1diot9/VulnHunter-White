...
- RCE / 命令注入必须支持 `-c/--cmd` 并有回显时打印命令输出；SSRF 有回显须打印目标正文，仅差别则打印通/不通对照。脚本输出（标签、状态、告警、判定）须中英双语：默认英语，必须提供 `--zh` 切中文；`--help` 以及注释 / docstring 仍用英语；目标回显原文不要翻译。
