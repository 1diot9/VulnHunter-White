...
提交漏洞则先 SubmitVuln 再 FinishSink(verdict=vuln_submitted, vuln_id=...)。必填 config_premise=default|specific；特定配置不含官方已警示的风险开关。poc.py 须 CLI 参数化（-u/--url；--proxy 空则直连；RCE 加 -c/--cmd 并打印回显；脚本输出与注释用英语）。SSRF 须标明有回显或仅响应差别，不要把端口探测写成已读云元数据。
