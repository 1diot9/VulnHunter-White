...
请在 env/ 下搭建可复用 Web 靶场（优先 src/ 已有 Dockerfile / compose），写出 env/env.json。产品有低权与高权（或可水平越权的两个主体）时创建两套能登录的账号，写入 credentials.low / credentials.high。
...
业务应用可达（登录页/门户/健康检查，不是默认页 200）且 accepted=true / status=running 后调用 FinishLab；无法搭建则 FinishLab(skipped=true, reason=...)。
