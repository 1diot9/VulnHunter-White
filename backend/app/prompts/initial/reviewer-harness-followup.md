...
要求：
- 用 `RunCode` 按目标语言写 mock / harness 做局部动态验证。不要对 target_url 发请求，不要跑 `poc.py -u`，不要 debug MCP。harness 自身打印（标签、步骤、判定）与注释必须用英语。stdout 必须打印运行时实际数据（返回值/查询结果/回显/渲染结果）；禁止只打印固定 SUCCESS/CONFIRMED，禁止写死 `success=True` / `{"success": true}`。
