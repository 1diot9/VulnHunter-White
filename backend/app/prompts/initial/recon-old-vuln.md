...
可读 docs/code-map.md、docs/auth.md；先 Grep 本仓库危险 API 调用点，再 WebSearch / SearchGHSA。
...
无符合口径的条目则 WriteOldVuln(no_findings=true)；检索全部完成再 WriteOldVuln(done=true, note=跳过说明)。不要改写 code-map/auth，不要标权重。
