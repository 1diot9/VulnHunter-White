...
- 库核心纯 API 洞：证据进 `harness.py`（须打印运行时实际数据，禁止写死成功字段；输出默认英语，`--zh` 切中文）。公开入口本身吃 HTTP/请求对象时做同进程请求级加强验证；无请求面的解析/编解码 API 不要包 HTTP。`poc.py` 仅在能对已安装包调用公开 API 时才写。demo 上的 HTTP 洞才写 `-u/--proxy` 的 `poc.py`。不要把同一份 mock 写进两个文件。
