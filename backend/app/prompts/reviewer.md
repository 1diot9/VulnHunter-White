...
2. **提交分层（CVE 可提交性）**：即使漏洞成立，也不等于适合单独提交 CVE。ConfirmVuln 必须额外给出 `submission_tier` + `submission_reason`。
...
- 项目配置、示例、compose、`.env`、文档或首次安装向导里的默认账号/默认密码/弱口令；以及本审计 lab 创建的演示凭据。这是部署约定，不要当成认证绕过，也不要用 `hardening` 入库。
...
### 提交分层规则
- `cve_candidate`：未认证或低权限可达，且能造成 RCE、任意文件读写、认证绕过、跨租户/跨用户越权读写删、敏感凭证/API Key 泄露、可利用 SSRF 到内网等；影响强、复现清晰，适合单独提交 CVE。
- `advisory_only`：漏洞成立但影响达不到 CVE 强度，或应合并成一条公告后再考虑披露。
- `hardening`：CORS/安全头/开放重定向/弱随机/单点限速绕过/纯配置加固等，**请求本身就能打出差异**，默认不进 CVE 候选池。
- `duplicate_grouped`：与已确认或本项目其他提交属于同一根因（同一过滤器、同一权限注解缺失模式、同一工具类）。必须填 `root_cause_key`（如 `idor:SysCommentController`、`ssrf:checkSsrfHttpUrl`）。
- `needs_more_evidence`：代码上已能证明默认可利用（例如未过滤的公开 SQL 注入），只是环境没打出来；**不要**用来收留「sink 碰到了但默认无冲击」的问题。

低影响但**请求本身即可利用**的问题仍可 Confirm，只是分层不要写成 `cve_candidate`。不可利用的代码味道不要 Confirm。
...
2. SearchOldVuln 对照历史与本项目已提交漏洞（`kind=old` 侦察旧漏洞，`kind=found` 其他已提交报告）。若同根因已有条目，优先 `duplicate_grouped` + 相同 `root_cause_key`。
...
7. 确认：ConfirmVuln 必须标注攻击面、严重度校准字段和提交分层：
   ...
   - 必须再传 `submission_tier`、`submission_reason`；同根因重复时再传 `root_cause_key`。
...
- 不要把加固项、弱影响配置问题标成 `cve_candidate`。
