...
2. 攻击者只凭题目允许的权限和用户可控输入（HTTP / WebSocket / RPC / MQ / 回调等），就能打出**可观察的有害冲击**（与正常请求可区分：读到不该读的**敏感**数据、写/删**敏感**对象、命令执行、未授权管理操作等）。读到公开静态资源、上传无害文件、或只改自己刚传的无害文件**不算**有害冲击。提交时必填 `config_premise`：`default`（**默认配置**即可利用）或 `specific`（须改应用自身提供的配置选项才可利用）。**特定配置不包括**官方文档已明确警示「开启后可能导致安全风险」的选项；仅在这类已警示开关下才成立的不要提交。须管理员先把攻击者控制的设备 / 邮箱 / Webhook / SNMP / unix-agent 源加进系统才有注入面时，`auth_premise` 必须写清「须管理员先加入该源，攻击者不能自己添加」，**禁止**写成无需登录 / 前台 / 未授权 / 普通用户即可注入。这不是前台洞：看谁能把攻击者的源加进系统，不要看轮询协议本身是否登录本应用，也不要看普通用户打开页面会不会中 XSS。
3. 不依赖第二个独立漏洞、不依赖审核员/攻击者先往服务器写 payload 文件、不依赖非默认目录布局（例如目标路径下碰巧存在 `templates/*.html`），也不依赖攻击者事先知道一个**不可获取且不可预测**的对象键（UUID / 随机 ID / 文件名 / 附件键等）。
...
- **不可获取且不可预测的对象键**（UUID / 随机 ID / 文件名 / 附件键等）：利用必须先知道该键，且攻击者**凭自身权限**无法从列表、搜索、同权/匿名 API、目录遍历拿到，也不能枚举或预测（短数字、时间戳、可遍历）。仅「知道键就能读/下/改」不够 → 丢弃。**不算可获取**：他人分享链接、邮件通知、业务页预览 URL、需先打开别人文档才可见的参数——不能假定攻击者已持有这类外链。即使有硬编码签名密钥可伪造 token，只要对象键仍只能靠上述外链碰巧泄露，仍应丢弃。能独立列出、可预测或可遍历则仍可提交。
...
- 项目配置、示例、compose、`.env`、文档或首次安装向导里的**默认账号/默认密码/弱口令**（含 `admin/admin`、文档演示凭据、本审计 lab 写入的账号）。这是部署约定，不是代码漏洞。例外：当作**服务端机密**的源码硬编码密钥**可以提交**（JWT/HMAC 签名密钥、接口签名 secret、私钥、第三方 API Key、保护库内/备份等本不该公开的服务端加解密密钥，写死在 `.java`/`.go`/`.py` 等程序文件中）——但若密钥危害仍依赖不可获取的对象键（例如只能给已知附件 fdId 伪造下载签名），按上条对象键闸门丢弃，不要单凭「硬编码密钥」提交。不要提交：`application.yml`、`.env`、compose 等用户可改配置里的口令；仅用于前端传输混淆的 AES/DES（密钥在前端 JS，或故意公开接口下发）；危害只是解开前端本就会解的字段或已拦截登录包的「硬编码密钥」。
- 已知且允许的业务能力（见 docs/auth.md）——若仍提交，必须 `intended_behavior=true`。
- 不要按漏洞类型填写或推断严重度；入库为 `pending`，由 Reviewer 填写 CVSS 3.1 与 CVSS 4.0 向量，分数由系统计算。
