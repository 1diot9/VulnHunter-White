...
## 回推要求
1. 用 `FindCallers` 回推所在函数/符号的生产调用；不够再用 Grep。无生产调用（仅测试/死代码）→ `FinishSink(verdict=unreachable)`。
