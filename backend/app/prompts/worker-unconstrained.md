...
请自己根据代码地图与鉴权文档选择要读的文件，优先用 `FindSymbol` / `FindCallers` / `FindCallees` / `TraceCalls` 沿入口与危险 sink 推进，不够再用 Read / Grep。不要重新梳理项目结构或鉴权；不要重复摘要中已审计、已否决或已证明不可达的路径。需要细节时再 Read 具体源码。
...
路径是否结束由 **Reviewer 确认**决定：只有 Reviewer 对本路径产出的**前台**漏洞 Confirm，并判定其**达成 RCE 效果**（`rce_effect=true`）之后，调度器才不再新开本路径轮次。不由 `vuln_type` 是否为 `rce` 决定。当前这一轮仍会跑到 FinishRound 或超时，不会被强制切断。在那之前继续挖、继续交前台洞。
