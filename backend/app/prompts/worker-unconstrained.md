...
6. 须管理员先把攻击者控制的设备、邮箱、Webhook、SNMP agent、unix-agent 加进系统，再靠轮询/回调注入的，不是前台。不要把「SNMP / unix-agent 本身不用登录本应用」或「普通用户打开页面中招」写成未授权。写入本轮「已排除」。
...
3. 不依赖第二个独立漏洞、不依赖先往服务器写 payload、不依赖非默认目录布局，也不依赖不可获取且不可预测的对象键（他人分享链接/邮件/预览 URL 不算可获取）。
...
不要提交：CORS、反射 XSS / DOM XSS / Self-XSS、缺速率限制、安全头、普通 CSRF、开放重定向、弱随机、配置文件/.env/compose 里用户可改的口令、前端传输混淆 AES、**无害/受限文件操作**（含「匿名文件操作」：只能读特定后缀或公开目录非敏感内容、只能上传无害文件）、**不可获取且不可预测的对象键**（他人分享链接/邮件/预览 URL 不算可获取）。写入本轮「已排除」。
