...
- **匿名 SQLi 泄露 admin 凭据 → 登录 → ping 命令注入 RCE**（#183, #184）→ `docs/attack-chains/匿名-SQLi-泄露-admin-凭据-登录-ping-命令注入-RCE.md`
  - 匿名用户通过未授权 SQLi 拖取 admin 明文密码，登录获取 admin 会话后利用 ping 接口命令注入实现 RCE，从匿名访问到完全控制服务器。

## 其他简述

- **存储型 XSS 劫持 admin 浏览器 → 同源 fetch 触发 ping RCE**（#185, #184）
