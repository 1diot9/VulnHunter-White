...
**流水线概要**：Recon（代码地图、鉴权、历史漏洞、文件定权）完成后开始挖掘；创建时可勾选代码库（CodeGraph 源码图 + 可选 Jar Analyzer 业务 jar 图，默认关），开启后与 Recon 并列、地图 Agent 点名后端并构建完成后才挖掘 → 启发式 / 快速扫描 / 历史漏洞绕过 / 无约束扫描（至少一条）→ Reviewer（默认静态；可选靶场动态或局部 harness）→ 可选 Verifier（FOFA）与攻击链串联。

**设计文档**（架构、阶段设计、容错机制、创新点）：[`docs/DESIGN.md`](docs/DESIGN.md)

Windows 用根目录 `start.cmd` / `stop.cmd`；**Linux / macOS** 用 `sh start.sh` / `sh stop.sh`。

**Docker 发行版（可选）**：Windows 用 `docker\desktop\start.cmd`（Docker Desktop + WSL2）；Linux 用 `sh docker/desktop/start.sh`（Docker Engine）。停止用同目录 `stop.cmd` / `stop.sh`。详见 [Docker 一键启动（Windows / Linux）](#docker-一键启动)。
...
- [启动前要做的构建（必看）](#启动前要做的构建)
  ...
  - [8. 可选环境变量](#8-可选环境变量)
- [Docker 一键启动（Windows / Linux）](#docker-一键启动)
- [一键启停](#一键启停)
  - [首次打开：设置页](#首次打开设置页)
- [手动启动](#手动启动)
  - [后端](#后端)
...
- 挖掘与审核结束后可选**攻击链串联**
- 项目详情可将勾选的产出对照历史漏洞与最新源码去重（已公开或已修复）
- 设置页可配**模型商池**；每项目可设 Token 用量上限；发现仓库可按提示词搜索或从公开 GHSA 筛选
...
发现仓库页面：可按用户提示词搜索 GitHub 仓库，也可从公开 GHSA 筛可审计仓；会排除官方演示、示例工程和学习用项目。已创建与可创建分开列出，支持单条或一键移除。搜索默认限时 600 秒，超过 5 个时每多 1 个加 60 秒；超时会说明原因并保留已找到的仓库。

## 成果展示

| 仓库名称                          | 漏洞编号                                                     | 漏洞类型     | CVSS评分 |
| --------------------------------- | ------------------------------------------------------------ | ------------ | -------- |
...
| vuejs/devtools                    | NCC-2026-09794                                               | RCE          | 7.3      |
| vuejs/devtools                    | NCC-2026-09797                                               | RCE          | 7.5      |
| vuejs/devtools                    | NCC-2026-09800                                               | RCE          | 8.7      |
| vuejs/devtools                    | NCC-2026-09857                                               | 原型链污染   | 8.8      |
| vuejs/devtools                    | NCC-2026-09876                                               | 未授权访问   | 8.7      |
| 蓝凌EKP                           | NCC-2026-10030                                               | 信息泄露     | 6.9      |
| 蓝凌EKP                           | NCC-2026-10032                                               | 认证绕过     | 6.9      |
| xorbitsai/inference               | NCC-2026-10126                                               | 认证绕过+RCE | 9.3      |
...
| vllm-project/vllm                 | [GHSA-5fj9-pfhr-6j48](https://github.com/vllm-project/vllm/security/advisories/GHSA-5fj9-pfhr-6j48) | DoS          | 5.9      |
| librenms/librenms                 | [GHSA-ffjc-4fr5-47c6](https://github.com/librenms/librenms/security/advisories/GHSA-ffjc-4fr5-47c6) | XSS          | Low      |
| AsyncHttpClient/async-http-client | [GHSA-qjr7-w8pj-pmv9](https://github.com/AsyncHttpClient/async-http-client/security/advisories/GHSA-qjr7-w8pj-pmv9) | 会话固定     | 4.0      |
| AsyncHttpClient/async-http-client | [GHSA-2jwh-9rmr-j4xf](https://github.com/AsyncHttpClient/async-http-client/security/advisories/GHSA-2jwh-9rmr-j4xf) | 认证不当     | 6.8      |
...
| ---------------------------- | ------------ | ------------------- | ------------ | ---- |
| https://github.com/movon-ava | getgrav/grav | GHSA-mwjj-r7vm-pgqm | 权限提升     | 8.8  |
| https://github.com/movon-ava | getgrav/grav | GHSA-pg8v-xw58-frq8 | 敏感文件泄露 | 7.5  |
...
应用跑在 Linux 容器里，通过挂载 `docker.sock` 在宿主引擎上构建并运行局部验证沙箱（DooD）。**不**自动搭建被测应用的 Docker 靶场；验证方式为关闭 / 局部验证 / **人工靶场**。本机根目录 `start.cmd` / `start.sh` 仍可使用完整靶场动态。

要求：

- **Windows**：Docker Desktop 已安装并正在运行（托盘可见，WSL2 后端）
- **Linux**：Docker Engine + Compose v2 插件已安装并正在运行（`sudo systemctl start docker`）；当前用户能无交互执行 `docker version` / `docker compose`（加入 `docker` 组后重新登录，不要用 sudo 起本脚本）
...
浏览器打开 `http://127.0.0.1:16788`（前后端同端口；静态 UI 由 FastAPI 托管）。

说明：

- 数据目录仍是仓库根下的 `data/`（与本机启动共用）。Linux 下容器按当前用户 uid 运行，避免 `data/` 变成 root 所有
- 人工靶场可填 `http://127.0.0.1:<端口>`，容器内会改写为 `host.docker.internal`（Linux 通过 `host-gateway` 注入）
- 默认端口 **16788**（与本机 `start.cmd` / `start.sh` 的 16780 错开，可并行）；可用环境变量 `VULNHUNTER_PORT` 覆盖
- amd64 / arm64 均可构建应用镜像。Rootless Docker 会尝试 `$XDG_RUNTIME_DIR/docker.sock`；SELinux Enforcing 时启动脚本会给数据盘加 `:z` 并关闭容器 MCS 标签
- macOS Docker Desktop 也可试 `sh docker/desktop/start.sh`；未作为第一期验收平台
...
| 代码库（调用图查询） | 创建项目时勾选；本机 `codegraph` / `jar-analyzer-engine`，或设置页路径 / env | 未勾选则不建图、不占磁盘，挖掘只等侦察。勾选后由地图 Agent `MarkCodeIntel` 点名 CodeGraph 与/或 Jar Analyzer；缺失工具会自动装到 `data/tools/`；失败则降级为 Read/Grep，不阻塞挖掘 |
...
| Docker 版 Linux：`permission denied` 访问 docker.sock | 把用户加入 `docker` 组后重新登录：`sudo usermod -aG docker $USER`；不要用 sudo 跑 `start.sh` |
...
| 项目与挖掘配置 | 创建时选择赏金（默认）/ 全量 / 自定义模式；勾选挖掘路径：启发式（默认开，可开轻量版只挖权重 100）、快速扫描（默认关）、历史漏洞绕过（默认关）、无约束扫描（默认关），至少开一条。每项目可单独选模型，不选则用设置页全局模型；可设 Token 用量上限；可粘贴或上传文本作为 Worker 额外人工提示。发现仓库页可填提示词优先按意图搜仓，留空则从公开 GHSA 筛，并排除演示/学习项目；可单条或一键移除候选。搜索默认 600 秒，超过 5 个时每多 1 个加 60 秒。GitHub 项目从暂停续跑时会先检查并同步上游最新代码（成功会在列表与详情提示已拉取的提交；失败则仍用当前快照）；zip 不受影响 |
...
| 代码库 | 创建时可选，默认关。开启后与 Recon 并列。地图 Agent 点名后端后构建：CodeGraph 索引 `src/`；Jar Analyzer 仅对点名业务 jar 建字节码图。失败降级为 Read/Grep。源码变化标过期，由用户点重建。关闭会删除该项目索引。Worker / Reviewer 可用同一套调用图短查询（平台路由） |
...
| 产出漏洞去重 | 项目详情「本项目漏洞」勾选后对照历史漏洞与最新源码：已公开或已修复分别标误报；日志在阶段日志「产出去重」 |
| 容错与调度 | LLM 按端点冷却并换路续跑、超时 Conclude、死循环新开、阶段最多再试 2 次；模型商池各 Base URL 未禁用端点并发之和为全局 LLM 线程上限（单端点默认 6），新会话均匀分配、同一轮对话优先同模型（提高缓存命中）并尽量粘滞原端点、超出按到达顺序排队；设置页可禁用端点；项目暂停后释放名额、续跑再排队；同一端点发请求默认最少间隔 2 秒（可关），排队不计超时；429 / 额度用尽只冷却该端点，结束后重新参与分配 |
| 历史漏洞 | 先 GHSA / GitHub Issues 爬虫落盘（第一阶段禁止 WebSearch），再 WebSearch 补漏；只收集不读源码。公开洞标 `patched`，未修复来自未关闭 Issues（`unpatched`） |
| 设置与运维 | 手动清理 X 天前 SSE 实时日志；CLI 工具目录（默认 `tools/cli`）供 Reviewer `SearchTools` 检索后 Shell 执行；可配置 CodeGraph / Jar Analyzer 路径 |
