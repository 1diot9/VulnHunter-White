...
| 动态验证 | 创建时默认关闭（仅静态复核）。开启后 Reviewer 搭 Docker 靶场并跑 HTTP PoC（`poc.py -u`）；靶场可用时 `ConfirmVuln` 系统再跑落盘 `poc.py`，退出码非 0 拒绝确认。PoC 由 Reviewer 收口；缺失或跑不通且需改写时才用 Java / Node / Python debug MCP。`poc.py` 须支持 `-u/--url`、`--proxy`（空则直连）、RCE 的 `-c/--cmd`；`poc.py` / `harness.py` 自身打印与注释用英语 |
