...
- 可选攻击链串联在创建时由用户决定，默认关闭；创建后可在项目设置中开启。开启后，在挖掘完成且审核队列清空后启动一轮 Agent：用 `SearchOldVuln`（仅本项目已确认产出）+ `Read`/`Grep` 推理多步串联，经 `SubmitAttackChain` 写入 `docs/attack-chains/`；已确认洞少于 2 条时跳过。与 Verifier 互不依赖，可并行；`project_complete_gates` 在开启时须等 `attack_chain_done`。重置启发式挖掘进度会清 `attack_chain_done` 以便再跑。
